...
Strategy decision (addressing open question #5 - scoring approach):
- Use config-driven keyword matching rather than embeddings. Rationale:
  * Fully deterministic and auditable (same input always produces same score).
  * No model inference cost or latency.
  * Sufficient for the keyword-based mapping vocabulary we have.
  * Embeddings would add latency, cost, and non-determinism for marginal benefit
    at this stage. Can be layered in later if keyword matching proves too coarse.
- Weights are loaded from weights.yaml and can be tuned without code changes.
- Fit reasons are templated strings tied to the scoring sub-components that
  contributed most. They explicitly reference user goals, preferences, and
  measurable provider attributes.

Scoring formula:
  total = sum(weight_i * subscore_i) for all sub-score components
  Each sub-score is normalized to [0, 100].
  Final score = round(total) clamped to [0, 100].
"""
...
# Medical categories that count as "medical" care_style alignment
...
def _keyword_overlap(text_tokens: set[str], reference_terms: list[str]) -> float:
    ...
    for term in reference_terms:
        ...
        # Check if any word of the term appears in the text tokens
...
def _compute_outcome_alignment(
    ...
    Uses config mappings to determine which specialties/modalities are relevant
    for each outcome keyword, then checks if the candidate matches.
    """
    ...
    # Build a set of what this candidate offers
    ...
    for outcome in goal_outcomes:
        ...
        for keyword, modal_map in cfg.outcome_to_modalities.items():
            if keyword in outcome_lower:
                # Get relevant modalities for this keyword
                relevant = []
                for style_key in ("medical", "whole_health"):
                    relevant.extend(modal_map.get(style_key, []))

                # Check if candidate matches any relevant modality
        ...
        # Direct keyword check in candidate signals
...
def _compute_symptom_alignment(
    ...
    """Score symptom/diagnosis alignment using symptom_to_specialties config."""
    if not main_issue and not diagnoses:
    ...
    search_terms = []
    if main_issue:
        search_terms.append(main_issue.lower())
    search_terms.extend(d.lower() for d in diagnoses)
...
def _compute_care_style_alignment(
    ...
    """Score how well the candidate's category matches the requested care style."""
    category = candidate.get("category", "other")

    if care_style == "medical":
    ...
    if care_style == "whole_health":
        if category in _WHOLE_HEALTH_CATEGORIES:
            return 100.0
        return 20.0

    # mixed: both are welcome
...
def _compute_preference_alignment(
    ...
    # Apply avoidance boosts (whole_health providers get boosted when user
    # wants to avoid medication/surgery)
...
def _compute_review_quality(candidate: dict[str, Any]) -> float:
    ...
    A provider with 4.5 stars and 100 reviews scores higher than
    5.0 stars and 2 reviews.
    """
...
def _compute_accessibility(
    ...
    # so this is a minor signal. If user wants in_person and provider is nearby,
    # that already works. If telehealth, we can't verify, so no penalty.
    if visit_mode == "telehealth":
        # Slight reduction for in-person-only signals (distance less important)
...
def score_candidate(
    ...
    sub_scores = {
        ...
        "symptom_alignment": _compute_symptom_alignment(main_issue, diagnoses, candidate),
        "care_style_alignment": _compute_care_style_alignment(care_style, candidate),
...
def generate_fit_reasons(
    ...
    Each reason explicitly ties back to user inputs. Never fabricates claims.
    """
    reasons: list[str] = []

    # Outcome alignment reasons
    if sub_scores["outcome_alignment"] >= 60.0:
        # Find which outcomes match best
        for outcome in goal_outcomes[:2]:
    ...
    elif distance is not None and sub_scores["accessibility"] >= 60.0:
        reasons.append(f"{distance} km from your location")
